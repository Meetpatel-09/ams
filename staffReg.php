<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
          ...
          if (empty(trim($_POST["inputPhonenumber"]))) {
               ...
               $query = "SELECT id FROM staff WHERE p_number = ?";
          ...
          if(empty($fname_err) && empty($address_err) && empty($email_err) && empty($password_err) && empty($con_password_err) && empty($mobile_err)) {
               ...
               $target = "staffPhoto/".basename($image);
               move_uploaded_file($_FILES['exampleFormControlFile1']['tmp_name'], $target);

               $query = "INSERT INTO staff(f_name,email,p_number,address,type,b_name,photo,password) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
               $stmt = mysqli_prepare($conn, $query);
               if($stmt) {
                    mysqli_stmt_bind_param($stmt, "ssssssss", $param_fname, $param_email, $param_p_number, $param_address, $param_type, $param_b_name, $param_photo, $param_password);
          ...
          <div class="col-md-10">
               ...
               <div class="col-sm-6 col-md-4">
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                         ...
                         <div class="col-md-6">
                              <h6>Type</h6>
                              <select id="inputTypeOfStaff" name="inputTypeOfStaff" class="selecter_3 form-control">
                         ...
                         <div class="col-md-6">
                              <h6>Branch</h6>
                              <select id="inputBranch" name="inputBranch" class="selecter_3 form-control">
                                   <option selected>Choose</option>
                                   <option>Civil</option>
